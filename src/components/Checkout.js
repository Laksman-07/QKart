...
/**
 * Returns the complete data on all produitems by searching in productsData
 *
 * @param { String } token
 *    Login token
 *
  ...
  return (
    <>
      ...
      <Grid container>
          ...
          <Box className="shipping-container" minHeight="100vh">
            ...
            <Box>
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               addresses.all.map((item)=><Box className={`address-item ${addresses.selected===item._id? "selected":"not-selected"}`} onClick={()=>setAddresses({ ...addresses, selected: item._id })}>
                  <Typography>
                    {item.address}
                  </Typography>
        ...
        >
          <Box color="#3C3C3C" alignSelf="center" fontWeight="900" paddingBottom="1rem">
            Order Details
          </Box>
          <Box color="#3C3C3C" alignSelf="center" display="flex" justifyContent="space-between" paddingBottom="0.5rem">
